...
string Swapper(string line)
{
	...
	if (dividersCount > 0)
	{
		...
		for (int i = 0; i < dividersCount;)
		{
			...
			for (int j = beginFrom; j <= dividers[i]; j++)
			{
				word1 += limitedLine[j];
			}
			...
			if (i + 1 < dividersCount)
			{
				...
				for (int j = dividers[i] + 1; j <= dividers[i + 1]; j++)
				{
					word2 += limitedLine[j];
				}
				result += (word2 + word1);
				// В следующий раз первое слово будем считывать начиная с другого места
				beginFrom = dividers[i + 1] + 1;
				i += 2;
			}
			// Если это слово - последнее, то дописываем
			else
			{
				for (int k = dividers[dividersCount - 1] + 1; k < limitedLine.length(); k++)
				{
					word2 += limitedLine[k];
				}
				result += (word2 + " " + word1);
